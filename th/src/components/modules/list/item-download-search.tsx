...
type Props = {
    listItemDownloadSearch: LayoutListDownloadSearch[],
}

export default function LayoutItemDownloadSearch({ listItemDownloadSearch }: Props) {
    return (
        <>
            <div className="layout-item-search">
                <ul className="item-list">
                    {listItemDownloadSearch.map((item) => (
                        <li key={item.id} className="fluid">
                            <div className="wrapper">
                                ...
                                <div className="content">
                                    <Typography variant="h3" className="h-title text-limit">
                                        <strong>
                                            {item.hTitle} 
                                            {item.hNumber && (
                                                <span className="color"> {item.hNumber}</span>
                                            )}
                                        </strong>
                                    </Typography>
                                    {item.hDesc && (
                                        <Typography variant="body1" className="h-desc text-limit desc">
                                            {item.hDesc}
                                        </Typography>
                                    )}
                                    {item.infoList && (
                                        <div className="info">
                                            {item.infoList.map((list) => (
                                                <Typography key={list.id} variant="body1" className="desc">
                                                    <strong>{list.title} :</strong> {list.desc}
                                                </Typography>
                                            ))}
                                        </div>
                                    )}
                                    {item.boxList && (item.boxList.map((list) => (
                                    ...
                                    {(item.tagDate || item.tagInfo || item.tagBlack || item.tagRed) && (
                                        <div className="tag">
                                            {item.tagDate && (<div className="item date">{item.tagDate}</div>)}
                                            {item.tagInfo && (<div className="item black">{item.tagInfo}</div>)}
                                            {item.tagBlack && (<div className="item black">คดีหมายเลขดำที่ {item.tagBlack}</div>)}
                                            {item.tagRed && (<div className="item red">คดีหมายเลขแดงที่ {item.tagRed}</div>)}
                                        </div>
                                    )}
                                </div>
                                {item.linkUrl && item.btnUrl &&(
