...
// types
import type {detailDownload} from "@/types/detail";

type Props = {
    detailDownload: detailDownload[],
}

export default function LayoutDetailDownload({ detailDownload }: Props) {
    return (
        <div className="layout-download">
            <ul className="item-list">
                {detailDownload.map((item) => (
                    <li key={item.id} className={`${item.column === 'fluid' ? 'fluid' : ''}`}>
                        <div className="wrapper">
                            ...
                            <div className="action">
                                <Link href={item.linkUrl} className="btn btn-secondary" title="ดาวน์โหลดเอกสาร">
                                    <div className="ic">
